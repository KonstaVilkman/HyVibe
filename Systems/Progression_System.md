# Progression System

**Purpose:** Long-term goals that keep players engaged and create aspiration.
**Status:** Concept Phase
**Last Updated:** January 2026

---

## Core Concept

Progression is about **giving players reasons to keep playing**. Every action should feel like it contributes to growth.

**Key Principles:**
- Multiple progression tracks (not just one number)
- Always something to work toward
- Visible progress creates satisfaction
- Milestones unlock meaningful rewards + Lootboxes
- Realistic timelines based on actual play

---

## Progression Tracks

| Track | What It Measures | Primary Benefit |
|-------|------------------|-----------------|
| **Player Level** | Overall account progress | Unlocks features, housing tiers |
| **Fame** | Social status/activity | Hosting bonuses, titles |
| **Collections** | Items gathered | Completion bonuses |
| **Achievements** | Milestones completed | Gems, Lootboxes, cosmetics |

---

## Player Level

### No Level Cap

**Players can grind as much as they want.** There is no maximum level. High levels become a flex and show dedication.

### How You Earn XP

| Source | XP Amount | Notes |
|--------|-----------|-------|
| Time online (passive) | +5 XP/min | Always earning while online |
| Minigame played | +20-40 XP | Participation |
| Minigame won | +50-80 XP | Skill reward |
| Daily login | +200 XP | Show up bonus |
| Achievement unlocked | +100-500 XP | One-time spikes |
| Event participation | +50-200 XP | Engagement reward |

**Passive XP:** All players earn 5 XP per minute just for being online. Simple and universal - no special zones required.

### XP Curve

| Level Range | XP Per Level | Notes |
|-------------|--------------|-------|
| 1-10 | 500 | Fast early progress |
| 11-25 | 750 | Still accessible |
| 26-50 | 1,000 | Steady climb |
| 51-100 | 1,500 | Committed players |
| 101-200 | 2,000 | Dedicated grinders |
| 201-500 | 2,500 | True veterans |
| 501+ | 3,000 | Legends |

### Level Time Estimates

Based on 5 XP/min passive + active play (avg ~500-800 XP/hour):

| Level | Approx. Time | Playstyle |
|-------|--------------|-----------|
| 10 | 6-10 hours | New player milestone |
| 25 | 20-30 hours | First week player |
| 50 | 50-70 hours | Regular player |
| 100 | 120-160 hours | Committed player |
| 200 | 300-400 hours | Dedicated grinder |
| 500 | 1,000+ hours | Legend status |

**No Cap Philosophy:** Let players grind. High levels become meaningful status symbols. Someone at Level 500+ has serious dedication.

---

## Level Unlocks

### Early Game (1-25)

| Level | Unlock |
|-------|--------|
| 1 | Tutorial complete, starter apartment |
| 5 | First job efficiency bonus, minigames unlocked |
| 10 | Apartment Tier 2 available ($3,000), first cosmetic set |
| 15 | All minigames unlocked, title customization |
| 20 | First achievement tier complete bonus |
| 25 | Loft Tier 3 available ($10,000) |

### Mid Game (26-60)

| Level | Unlock |
|-------|--------|
| 30 | Exclusive emotes |
| 35 | Elite cosmetic tier access |
| 40 | Penthouse Tier 4 available ($30,000), "Veteran" badge |
| 50 | "Dedicated" badge |
| 60 | Mansion Tier 5 available ($75,000), "Champion" title |

### Late Game (61-100)

| Level | Unlock |
|-------|--------|
| 75 | "Elite" title |
| 100 | "Centurion" title, special particle effect |

### Grinder Tiers (100+)

| Level | Unlock |
|-------|--------|
| 150 | "Master" title |
| 200 | "Grandmaster" title, unique visual effect |
| 300 | "Legend" title |
| 500 | "Mythic" title, permanent recognition |
| 1000 | "Immortal" title, monument in city |

**Beyond 1000:** Levels continue infinitely. Bragging rights only - the number itself is the flex.

---

## Milestone Rewards

**Every 5 levels = Milestone with guaranteed rewards**

### Levels 5-25

| Level | Cash | Gems | Lootbox | Special |
|-------|------|------|---------|---------|
| 5 | $300 | 10 | Common | — |
| 10 | $500 | 20 | Common | Cosmetic |
| 15 | $750 | 30 | Common | — |
| 20 | $1,000 | 40 | Rare | Title Option |
| 25 | $1,500 | 50 | Rare | Badge |

### Levels 30-60

| Level | Cash | Gems | Lootbox | Special |
|-------|------|------|---------|---------|
| 30 | $2,000 | 60 | Rare | — |
| 35 | $2,500 | 75 | Rare | Cosmetic Set |
| 40 | $3,500 | 100 | Epic | Veteran Badge |
| 45 | $4,500 | 125 | Epic | — |
| 50 | $6,000 | 150 | Epic | Dedicated Badge |
| 55 | $7,500 | 175 | Epic | — |
| 60 | $10,000 | 200 | Legendary | Champion Title |

### Levels 65-100

| Level | Cash | Gems | Lootbox | Special |
|-------|------|------|---------|---------|
| 65 | $12,000 | 225 | Epic | — |
| 70 | $15,000 | 250 | Epic | — |
| 75 | $18,000 | 275 | Legendary | Elite Title |
| 80 | $22,000 | 300 | Epic | — |
| 85 | $26,000 | 350 | Epic | — |
| 90 | $30,000 | 400 | Epic | — |
| 95 | $35,000 | 450 | Epic | — |
| 100 | $50,000 | 600 | Legendary | Centurion Title |

### Levels 100+ (Every 50 Levels)

| Level | Cash | Gems | Lootbox | Special |
|-------|------|------|---------|---------|
| 150 | $75,000 | 800 | Legendary | Master Title |
| 200 | $100,000 | 1,000 | 2x Legendary | Grandmaster Title |
| 250 | $125,000 | 1,200 | Legendary | — |
| 300 | $150,000 | 1,500 | 2x Legendary | Legend Title |
| 400 | $200,000 | 2,000 | 2x Legendary | — |
| 500 | $300,000 | 3,000 | 3x Legendary | Mythic Title |
| 1000 | $1,000,000 | 10,000 | 5x Legendary | Immortal Title + Monument |

**Beyond 1000:** Every 100 levels grants a Legendary Lootbox + scaling Cash/Gems.

---

## Passive Progression

### Base Passive XP

**Everyone earns 5 XP/min while online. No strings attached.**

- Active playing? +5 XP/min
- Standing still? +5 XP/min
- AFK anywhere? +5 XP/min

**Philosophy:** Your time matters. Whether you're grinding or chatting with friends, you're progressing. No special zones needed - just be online.

### Optional: Bonus Multipliers (From Gear/Items)

Players can boost their XP gain through equipment:

| Item Type | Max Bonus | Stacking |
|-----------|-----------|----------|
| Outfit | +10% XP | One equipped |
| Pet | +5% XP | One active |
| **Total Cap** | **+15% XP** | Cannot exceed |

**Example:**
- Base: 5 XP/min = 300 XP/hour
- With +15% gear: 5.75 XP/min = 345 XP/hour
- Doing jobs/minigames on top? Even faster

**No Power Creep:** These percentages are LOCKED. Future items have the same caps, just different aesthetics.

---

## Achievements

### Categories

| Category | Examples |
|----------|----------|
| **Progression** | Reach Level 10, 25, 50, 100 |
| **Social** | Host 10 parties, Get 100 apartment visits |
| **Collection** | Own 50 furniture items, Complete a set |
| **Exploration** | Visit all districts, Find hidden spots |
| **Events** | Participate in 5 events, Complete event challenges |
| **Wealth** | Earn $10,000 total, Reach $50,000 balance |

### Achievement Rewards

| Achievement Tier | Gems | Lootbox | Fame | Special |
|------------------|------|---------|------|---------|
| Bronze | 10 | Common | +10 | — |
| Silver | 25 | Common | +25 | — |
| Gold | 50 | Rare | +50 | Cosmetic |
| Platinum | 100 | Epic | +100 | Title |

### Hidden Achievements

**Secret achievements for discovery:**
- Find easter eggs
- Do unusual things
- Community-discovered secrets

Reward: Unique cosmetics, Epic Lootbox, bragging rights

---

## Collections

### What Are Collections?

**Groups of related items that reward completion.**

| Collection | Items | Completion Reward |
|------------|-------|-------------------|
| Fish | All fish species (25) | Aquarium furniture, title, Epic Lootbox |
| Furniture Sets | Complete themed sets | Set bonus effect + Rare Lootbox |
| Pets | Own 10 different pets | Pet paradise decoration |
| Outfits | Own 25 different outfits | Wardrobe furniture + 100 Gems |
| Event Items | All items from an event | Event badge + Legendary Lootbox |

### Collection Progress

```
Fish Collection: 12/25 Complete
[############.............] 48%

Reward at 100%: Golden Aquarium + "Master Angler" title + Epic Lootbox
```

---

## Daily/Weekly Goals

### Daily Challenges (5 per day)

| Example Challenge | Reward |
|-------------------|--------|
| Win 2 minigames | $100, 10 Gems |
| Visit 3 apartments | $50, 5 Fame |
| Fish for 10 minutes | $75, 5 Gems |
| Harvest a plant | $50, 5 Gems |
| Socialize for 30 minutes | $75, 5 Gems |

**Each Daily Challenge Completed:** 1 Common Lootbox
**All 5 Dailies Completed:** Bonus 10 Gems

### Weekly Challenges (8 per week)

Larger goals with better rewards:

| Example Challenge | Reward |
|-------------------|--------|
| Earn $5,000 Cash | $500, 25 Gems, Common Lootbox |
| Host a party with 10+ guests | 50 Fame, Rare Lootbox |
| Like 20 apartments | $200, 20 Fame, Common Lootbox |
| Open 10 Lootboxes | 1 Rare Lootbox |
| Win 10 minigames | $300, Epic Lootbox |
| Complete 25 daily challenges | 1 Epic Lootbox |
| Grow 5 plants to harvest | $400, Rare Lootbox |

**Weekly Completion Bonus:** Finish all 8 = 1 Epic Lootbox + 50 Gems

### Minigame Lootbox Rewards

In addition to Cash/XP rewards, minigames give lootboxes:

| Achievement | Reward |
|-------------|--------|
| Every 2 wins | 1 Common Lootbox |

**Design Note:** This creates a sustainable lootbox source for active players without requiring progression milestones.

---

## Login Streaks

### Overview

Login streaks reward **consistency over intensity**. Just show up each day to build your streak and earn escalating rewards.

### Daily Login Rewards

| Day | Reward |
|-----|--------|
| 1 | $200 + 25 XP |
| 2 | $300 + 50 XP |
| 3 | $500 + 75 XP + 5 Gems |
| 4 | $600 + 100 XP |
| 5 | $800 + 125 XP |
| 6 | $1,000 + 150 XP + 10 Gems |
| **7** | **1 Common Lootbox** + $1,500 |

### Extended Streak Milestones

| Milestone | Reward |
|-----------|--------|
| **14 days** | 1 Rare Lootbox + $3,000 |
| **21 days** | 1 Rare Lootbox + 50 Gems |
| **30 days** | 1 Epic Lootbox + Exclusive Title |

### Cozy Design Rules

| Rule | Implementation |
|------|----------------|
| **Grace Period** | 1 skip per week doesn't break streak |
| **Streak Cap** | Resets at 30 days, starts fresh cycle |
| **Claim Window** | 24 hours from last login |
| **No Punishment** | Missing days = no reward, but no penalty |

**Design Philosophy:** Login streaks should feel like a gentle bonus, not an obligation. The grace period prevents real-life interruptions from feeling punishing.

---

## Season Pass (Optional Future Feature)

### Free Track

| Tier | Reward |
|------|--------|
| 1 | $100 |
| 5 | Common Lootbox |
| 10 | $200, 10 Gems |
| 15 | Uncommon Furniture |
| 20 | Rare Lootbox |
| 25 | $500, 25 Gems |
| 30 | Cosmetic |
| 40 | Epic Lootbox |
| 50 | Season Badge, $1,000 |

### Premium Track ($9.99)

| Tier | Reward |
|------|--------|
| 1 | $200, Exclusive Outfit |
| 5 | Rare Lootbox |
| 10 | $400, 25 Gems |
| 15 | Rare Furniture |
| 20 | Epic Lootbox |
| 25 | $750, 50 Gems |
| 30 | Rare Cosmetic Set |
| 40 | Legendary Lootbox |
| 50 | Season Exclusive Set, 100 Gems |

---

## Incentive Summary

| "Why should I...?" | Incentive |
|--------------------|-----------|
| Keep playing? | Level unlocks, daily rewards, Lootboxes |
| Play minigames? | Cash, XP, lootboxes, leaderboard status |
| Host parties? | Fame, passive income, status |
| Decorate apartment? | Apartment value, visitors, flex |
| Participate in events? | Exclusive items, event Lootboxes |
| Trade? | Get items you want, make profit |
| Reach high level? | Prestige, unlocks, massive milestone rewards |

---

## Anti-Grind Philosophy

**Progression should feel rewarding, not exhausting.**

| Design Choice | Why |
|---------------|-----|
| Passive XP exists | Can progress without constant effort |
| Daily bonuses cap | No need to play 12 hours |
| Multiple tracks | Stuck on one? Progress on another |
| Catch-up mechanics | Events help newer players |
| No mandatory grind | All content eventually accessible |
| Lootboxes at milestones | Every milestone feels exciting |

---

## Open Questions

| Question | Considerations |
|----------|----------------|
| Exact XP numbers | Needs playtesting for feel |
| Season pass at launch? | Later feature? |
| Milestone reward scaling | Are high-level rewards exciting enough? |
| Level display format | Show exact level or use tiers for very high levels? |

---

*See [Fame_System.md](Fame_System.md) for Fame progression.*
*See [Housing_System.md](Housing_System.md) for apartment progression.*
*See [Economy_System.md](Economy_System.md) for Lootbox system details.*
