# Development

**Purpose:** How we're building HyVibe — roadmap, phases, and daily planning.

---

## What's In This Folder

| Document | Description |
|----------|-------------|
| [Development_Path.md](Development_Path.md) | Complete roadmap from current state to launch |
| [Phase_0A_Minimum_Viable_Lobby.md](Phase_0A_Minimum_Viable_Lobby.md) | Current phase - detailed MVP plan, setup guides |
| [Hytale_Research_2026.md](Hytale_Research_2026.md) | Platform capabilities and limitations research |
| [Daily_Plans/](Daily_Plans/_Overview.md) | Day-by-day planning and progress tracking |

---

## Current Status

| Metric | Value |
|--------|-------|
| **Current Phase** | Phase 0A - Minimum Viable Lobby |
| **Platform** | Hytale (Early Access, launched Jan 13, 2026) |
| **Developer Status** | Solo, 8 days experience, made Monster Energy mod |

---

## Phase Overview

| Phase | Goal | Duration | Status |
|-------|------|----------|--------|
| **0A** | Basic lobby with levels, fame, profiles | 2-3 months | **Current** |
| **0B** | Add first activity, basic economy | 2-3 weeks | Planned |
| **1** | Housing, job, full fame, lootboxes | 6-8 weeks | Planned |
| **2** | Full jobs, minigames, housing tiers | 8-12 weeks | Planned |
| **3** | Onboarding, moderation, achievements | 4-6 weeks | Planned |
| **4** | Live operations, events, growth | Ongoing | Future |

---

## Phase 0A Checklist (Current)

### Must Have
- [ ] Small lobby plaza (one zone)
- [ ] Basic spawn point
- [ ] Chat functionality
- [ ] Player profiles (name display)
- [ ] LuckPerms for basic ranks
- [ ] TheEconomy for Cash system
- [ ] Simple level display

### Should Have
- [ ] Fame display
- [ ] NPC shopkeepers
- [ ] Basic furniture/decoration
- [ ] Parkour course (one simple track)

---

## Development Philosophy

> **Don't wait until everything is perfect.** Get a basic lobby working, invite people, and grow the community while building features.

**Why this works:**
- Early feedback shapes development
- Community grows alongside the server
- Players feel invested in the project
- You stay motivated seeing real people enjoy it

---

## Estimated Timeline

| Milestone | Target |
|-----------|--------|
| Phase 0A Complete | 2-3 months |
| Phase 1 (Core Loop) | ~6 months total |
| Beta Ready | ~8 months total |
| Public Launch | ~10-12 months total |

---

*See individual documents for detailed plans.*
